...
class WebformModalComposite extends WebformCompositeBase
{
    ...
    private static function setButtonOptions(array &$elements, array $element, string $type): void
    {
        ...
        switch ($type) {
            case 'bootstrap':
                $elements['webform_modal_link']['#title'] = $element['#modal_button'] ?? t('Add');
                $elements['webform_modal_link']['#type'] = 'link';
                $elements['webform_modal_link']['#url'] = Url::fromRoute('webform_modal_composite.loader');
